...
def send_panel(request):
    pendentes_qs = Atendimento.objects.filter(status_enviado="N").order_by("-criado_em")
    total_pendentes = pendentes_qs.count()
    total_lote = min(total_pendentes, _LOTE_SIZE)

    if request.method == "POST":
        ids = list(pendentes_qs.values_list("pk", flat=True)[:_LOTE_SIZE])
        if not ids:
            messages.warning(request, "Nenhum atendimento pendente para envio.")
        else:
            for pk in ids:
                send_whatsapp_for_atendimento.delay(pk)
